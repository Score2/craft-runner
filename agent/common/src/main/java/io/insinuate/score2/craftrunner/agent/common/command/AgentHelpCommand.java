...
final class AgentHelpCommand {
    void execute(AgentCommandContext context, AgentCommandSender sender, String label) {
        ...
        context.send(sender, AgentCommandContext.YELLOW + "/" + label + " info [plugin]" + AgentCommandContext.GRAY + " (alias: hot-info)");
        context.send(sender, AgentCommandContext.YELLOW + "/" + label + " token");
        context.send(sender, AgentCommandContext.YELLOW + "/" + label + " js-status");
        context.send(sender, AgentCommandContext.YELLOW + "/" + label + " js-load");
