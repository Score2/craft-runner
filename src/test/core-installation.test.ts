...
test("core installation prepares direct jars and materializes shareable dirs", async () => {
  ...
  const env = envMetadata(root, core.id);

  await fs.mkdir(env.server_dir, { recursive: true });
  const materialized = await installation.materialize(core, env);
  ...
  const libraries = path.join(env.server_dir, "libraries");
  const cacheDir = path.join(env.server_dir, "cache");
...
function testConfig(root: string): CraftRunnerConfig {
  return {
    ...
    env_base_dir: path.join(root, "envs"),
...
function envMetadata(root: string, coreId: string): EnvironmentMetadata {
  const now = new Date().toISOString();
  return {
    id: "env-a",
    kind: "local",
    server_dir: path.join(root, "envs", "env-a", "server"),
    base_dir: path.join(root, "envs"),
    persistent: false,
